...
const registerController = async (req, res) => {
    ...
    if(isExsitUser) {
        ...
        const token = jwt.sign({id: newUser.id, email}, process.env.JWT_SECRET, {expiresIn: '1h'});
...
const loginController = async (req, res) => {
    ...
    const token = jwt.sign({ id: isExistUser.id, email }, process.env.JWT_SECRET, { expiresIn: '1h' });
    res.cookie("token", token, {
        httpOnly: true,
        secure: false,
        sameSite: 'lax',
    });
    res.status(200).json({ message: "User logged in successfully", token, user: isExistUser });
}
